...
const localVenvPythonUnix = path.join(process.cwd(), ".venv", "bin", "python");
const parentVenvPythonUnix = path.join(
  process.cwd(),
  "..",
  ".venv",
  "bin",
...
const localVenvPythonWindows = path.join(
  process.cwd(),
  ".venv",
  "Scripts",
  "python.exe",
);
const parentVenvPythonWindows = path.join(
  process.cwd(),
  "..",
  ".venv",
  "Scripts",
...
const pythonExe =
  ...
  (existsSync(localVenvPythonUnix)
    ? localVenvPythonUnix
    : existsSync(parentVenvPythonUnix)
      ? parentVenvPythonUnix
      : existsSync(localVenvPythonWindows)
        ? localVenvPythonWindows
        : existsSync(parentVenvPythonWindows)
          ? parentVenvPythonWindows
          : "python3");
...
const enabledBackendServices =
  backendProfiles[devProfile] ?? backendProfiles.full;

const reservedPorts = new Set();
const servicePorts = { ...preferredPorts };

for (const serviceName of enabledBackendServices) {
...
const services = serviceDefinitions.filter(
  (service) =>
    service.name === "web" || enabledBackendServices.includes(service.name),
);
